/* XIANGQI (Chinese Chess) — logic. window.XiangqiLogic.
   9 files x 10 ranks of intersections. Red (you) at the bottom, Black (rival) at the top;
   Red moves first. Full rules: chariot, horse (hobbled leg), elephant (river-bound, eye block),
   cannon (jump to capture), advisor & general (palace-bound), soldier (sideways past the river),
   plus the flying-general rule. Checkmate or stalemate the general to win. Alpha-beta AI. */
(function () {
  const W = 9, H = 10;
  const id = (r, c) => r * W + c;
  const rc = i => [Math.floor(i / W), i % W];
  const inB = (r, c) => r >= 0 && r < H && c >= 0 && c < W;
  const inPalace = (s, r, c) => c >= 3 && c <= 5 && (s === "b" ? r <= 2 : r >= 7);
  ...
  function pieceMoves(board, r, c) {
    ...
    const s = p.s, out = [];
    const add = (nr, nc) => { if (!inB(nr, nc)) return; const t = board[id(nr, nc)]; if (!t || t.s !== s) out.push(id(nr, nc)); };
    switch (p.t) {
  ...
  function kingPos(board, s) { for (let i = 0; i < W * H; i++) { const p = board[i]; if (p && p.s === s && p.t === "K") return i; } return -1; }
  ...
  function attacked(board, target, bySide) {
    for (let i = 0; i < W * H; i++) { const p = board[i]; if (p && p.s === bySide) { const [r, c] = rc(i); if (pieceMoves(board, r, c).includes(target)) return true; } }
    return false;
  }
  function inCheck(board, s) { const k = kingPos(board, s); if (k < 0) return true; return attacked(board, k, s === "r" ? "b" : "r") || generalsFacing(board); }

  function doMove(board, from, to) { const nb = board.slice(); nb[to] = nb[from]; nb[from] = null; return nb; }
  ...
  function applyMove(s, from, to) {
    ...
    const next = s.turn === "r" ? "b" : "r";
    const moverName = s.turn === s.you ? "You" : "Rival";
    let log = push(s.log, s.turn === s.you ? "you" : "ai", `${moverName} ${cap ? "captured" : "moved"}.`);
    let ns = Object.assign({}, s, { board, turn: next, last: { from, to }, moveNo: s.moveNo + 1, log, check: false });
  ...
  // ===== AI =====
  const VAL = { K: 10000, R: 600, C: 300, H: 280, A: 120, E: 120, S: 70 };
  ...
  function search(board, side, me, depth, alpha, beta) {
  ...
  function aiMove(s) {
    ...
    const me = "b", moves = legalMoves(s.board, me);
  ...
  window.XiangqiLogic = { W, H, makeInitial, pieceMoves, legalMoves, applyMove, aiMove, inCheck, rc, id, inPalace };
})();
