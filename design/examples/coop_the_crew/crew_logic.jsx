/* THE CREW — cooperative trick-taking logic. window.CrewLogic.
   3 crew (you + Vega + Orion). 40 cards: 4 colours × 1-9 plus 4 rockets (trump) 1-4.
   Each mission has a set of public TASK cards; each task is assigned to the crew member
   who must WIN that exact card in a trick. Follow suit; rockets trump. The crew wins when
   every task is fulfilled; if any task card is taken by the wrong member, the mission fails.
   (Tasks are assigned to the card's holder — the classic beginner setup — so missions stay
   solvable through clean cooperative play.) */
(function () {
  const SUITS = ["pink", "blue", "green", "yellow"];
  const NAMES = ["You", "Vega", "Orion"];
  let UID = 0;
  function card(suit, val) { return { id: ++UID, suit, val }; }
  ...
  function sortHand(h) {
    const order = { pink: 0, blue: 1, green: 2, yellow: 3, rocket: 4 };
    return h.slice().sort((a, b) => order[a.suit] - order[b.suit] || a.val - b.val);
  }

  function makeMission(missionNo) {
    ...
    const tasks = chosen.map(o => ({ cardId: o.c.id, suit: o.c.suit, val: o.c.val, assignee: o.pi, done: false }));
    return {
  ...
  function ledSuit(trick) { return trick.length ? trick[0].card.suit : null; }
  ...
  function push(log, t, x) { return log.concat([{ t, x }]).slice(-50); }
  ...
  // ===== cooperative AI =====
  function strength(c) { return c.suit === "rocket" ? 100 + c.val : c.val; }
  function winnerSoFar(trick) { return trick.length ? trickWinner(trick) : -1; }

  // is card c a task card (any assignee)? returns the task or null
  function taskOf(tasks, c) { return tasks.find(t => t.cardId === c.id) || null; }

  function aiChoose(s, player) {
    const hand = s.hands[player];
  ...
  function cardLabel(c) { return c.suit === "rocket" ? "Rocket " + c.val : suitName(c.suit) + " " + c.val; }
  function suitName(s) { return s.charAt(0).toUpperCase() + s.slice(1); }

  window.CrewLogic = { SUITS, NAMES, makeMission, legalCards, trickWinner, playCard, aiStep, aiChoose, cardLabel, suitName, sortHand };
})();
