/* SKULL KING — game logic.
   Ported from design/examples/trick_skull_king/skull_king_logic.jsx; the IIFE +
   window.SkullKingLogic global became ESM exports and a SkullKingState type. Move
   resolution, scoring, and the AI are unchanged.

   Pirate trick-taking with bidding. 10 rounds; round R deals R cards each. You vs an AI. */

export type Player = 'you' | 'ai'
export type Suit = 'parrot' | 'chest' | 'map' | 'roger'
export type Kind = 'suit' | 'escape' | 'pirate' | 'mermaid' | 'skullking'

export interface Card { id: number; kind: Kind; suit?: Suit; rank?: number }
export interface TrickCard { player: Player; card: Card }
export type Phase = 'bid' | 'play' | 'trickEnd' | 'roundEnd' | 'gameOver'

export interface RoundResult { bid: number | null; tricks: number; delta: number }
export interface RoundLogEntry { round: number; you: RoundResult; ai: RoundResult }
...
export interface SkullKingState {
  ...
  bids: { you: number | null; ai: number | null }
  tricksWon: { you: number; ai: number }
  bonus: { you: number; ai: number }
  scores: { you: number; ai: number }
  leader: Player
  firstLeader: Player
  turn: Player
  trick: TrickCard[]
  lastTrick: { cards: TrickCard[]; winnerPlayer: Player; bonus: number } | null
  pending?: { winnerPlayer: Player; winnerIdx: number; bonus: number } | null
  roundLog: RoundLogEntry[]
  log: LogEntry[]
  winner: Player | 'tie' | null
}

export const SUITS: Suit[] = ["parrot", "chest", "map", "roger"] // roger = Jolly Roger = trump (black)
export const SUIT_NAME: Record<Suit, string> = { parrot: "Parrot", chest: "Chest", map: "Map", roger: "Jolly Roger" }

let UID = 0
function card(o): Card { return Object.assign({ id: ++UID }, o) }
...
export function isLegal(c: Card, hand: Card[], trick: TrickCard[]) {
  return legalPlays(hand, trick).some(x => x.id === c.id)
}

// resolve a completed (or partial) trick — cards: [{player, card}]
export function resolveTrick(cards: TrickCard[]) {
...
export function makeInitial(firstLeader: Player = "you"): SkullKingState {
  return dealRound({
    round: 1,
    phase: "bid",
    hands: { you: [], ai: [] },
...
function dealRound(s: SkullKingState, round: number): SkullKingState {
...
function aiBid(hand: Card[], round: number) {
  let exp = 0
  for (const c of hand) {
...
export function submitBid(s: SkullKingState, youBid: number): SkullKingState {
...
export function playCard(s: SkullKingState, player: Player, cardId: number): SkullKingState {
  ...
  if (trick.length < 2) {
    const other: Player = player === "you" ? "ai" : "you"
...
export function collectTrick(s: SkullKingState): SkullKingState {
...
function roundDelta(bid: number | null, tricks: number, bonus: number, round: number) {
  let base: number, bonusApplies = false
...
function scoreRound(s: SkullKingState): SkullKingState {
  ...
  if (s.round >= 10) {
    const winner: Player | 'tie' = scores.you === scores.ai ? "tie" : scores.you > scores.ai ? "you" : "ai"
...
export function nextRound(s: SkullKingState): SkullKingState {
  if (s.phase !== "roundEnd") return s
  const firstLeader: Player = s.firstLeader === "you" ? "ai" : "you"
  const ns = Object.assign({}, s, { firstLeader })
  return dealRound(ns, s.round + 1)
}

// ===== AI driver =====
function aiPlayChoice(s: SkullKingState): Card {
  ...
  if (s.trick.length === 1) {
    ...
    const evals = legal.map(c => {
      const res = resolveTrick(s.trick.concat([{ player: "ai", card: c }]))
      return { card: c, wins: res.winnerPlayer === "ai", bonus: res.bonus, val: cardVal(c) }
    })
    if (wantWin) {
...
export function aiStep(s: SkullKingState): SkullKingState {
  if (s.phase !== "play" || s.turn !== "ai") return s
  const c = aiPlayChoice(s)
  return playCard(s, "ai", c.id)
}
