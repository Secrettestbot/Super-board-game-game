/* NINE MEN'S MORRIS — UI (built for this codebase). A carved board (three concentric
   squares + cross-connectors) on the framework shell, vs an alpha-beta AI. Phase 1 click
   to place; phase 2 click your man then an adjacent point to slide; on a mill, click a
   highlighted rival man to remove. */
...
export function Morris() {
  const [s, setS] = useState<MorrisState>(() => MM.makeGame())
  const [showRules, setShowRules] = useState(false)
  const [sel, setSel] = useState<number | null>(null)

  function newGame() { setS(MM.makeGame()); setSel(null); setShowRules(false) }

  const aiActive = !s.winner && s.turn === 'b'
  useAITurn(aiActive, () => setS(p => MM.aiMove(p)), { delayMs: 520, tick: s.phase })
  ...
  const movableMen = useMemo(() => {
    ...
    return new Set(MM.legalSlides(s.board, 'w').map(([from]) => from))
  }, [moving, s.board])

  const removeTargets = useMemo(() => {
    if (!removing) return new Set<number>()
    return new Set(MM.removable(s.board, 'b'))
  }, [removing, s.board])

  function clickPoint(p: number) {
    if (placing) { if (s.board[p] === null) setS(MM.place(s, p, 'w')); return }
    if (removing) { if (removeTargets.has(p)) setS(MM.remove(s, p, 'w')); return }
    if (moving) {
      if (s.board[p] === 'w' && movableMen.has(p)) { setSel(p === sel ? null : p); return }
      if (sel !== null && slideTargets.has(p)) { setS(MM.slide(s, sel, p, 'w')); setSel(null); return }
    }
  }

  const lastSet = new Set(s.last)
  const tot = MM.counts(s)

  let banner: string, bk = ''
  if (s.winner === 'w') { bk = 'win'; banner = 'You win — the rival is down to two men' }
  else if (s.winner === 'b') { bk = 'lose'; banner = 'The rival wins' }
  else if (removing) { bk = 'you'; banner = 'Mill! Take a rival man' }
  else if (placing) { bk = 'you'; banner = `Your turn — place a man (${s.hand.w} in hand)` }
      ...
      >
        ...
        <div className="side">
          <div className="panel mm-score">
            <div className={'mm-pl w' + (s.turn === 'w' && !s.winner ? ' on' : '')}>
              <span className="mm-chip w" />
              <span className="mm-pl-name">You · White</span>
              <span className="mm-pl-stat">{s.onBoard.w}<span className="mm-sub"> on</span> · {s.hand.w}<span className="mm-sub"> hand</span></span>
            </div>
            <div className={'mm-pl b' + (s.turn === 'b' && !s.winner ? ' on' : '')}>
              <span className="mm-chip b" />
              <span className="mm-pl-name">Rival · Black</span>
              <span className="mm-pl-stat">{s.onBoard.b}<span className="mm-sub"> on</span> · {s.hand.b}<span className="mm-sub"> hand</span></span>
            </div>
            <div className="mm-tot">You {tot.wTotal} men · Rival {tot.bTotal} men</div>
          </div>
...
function ResultModal({ s, onNew }: { s: MorrisState; onNew: () => void }) {
  ...
  return (
    ...
    >
      <div className="finalsc"><span className="you">You {tot.wTotal}</span><span className="foe">Rival {tot.bTotal}</span></div>
    </Modal>
  )
}
  ...
  return (
    <Modal eyebrow="How to play" title="Nine Men's Morris" onClose={onClose}
      ...
      <div className="modal-body">
        <p>You are <b>White</b> and place first. Each side has <b>nine men</b>. In the <b>placing phase</b> you take turns dropping a man on any empty point.</p>
