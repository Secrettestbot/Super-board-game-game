/* SEVEN WONDERS DUEL — UI (built for this codebase).
   Draft cards from a three-age pyramid against a greedy AI on the framework shell.
   Only accessible (uncovered) cards are highlighted; click one to choose BUILD /
   raise a WONDER / DISCARD. The shared military track sits between the tableaus,
   and a science strip warns when either side nears the six-symbol win. The AI
   drafts one card per turn over all three ages; the useAITurn driver re-arms on
   s.step so its chained turns fire. UI shows the end state by default.
*/
...
export function SevenWondersDuel() {
  const [s, setS] = useState<SWDState>(() => G.makeGame())
  const [showRules, setShowRules] = useState(false)
  // Currently selected accessible card id (player 0 picks an action for it).
  ...
  // AI drafts one card per turn across all three ages; re-arm on s.step.
  useAITurn(s.winner == null && s.turn === 1, () => setS((p) => G.aiTurn(p)), { delayMs: 640, tick: s.step })
  ...
  function doBuild() {
    ...
    if (!G.canAfford(s, 0, selCard)) return
    setS(G.buildCard(s, selCard.id)); setSel(null)
  }
  function doDiscard() {
    if (!yourTurn || !selCard) return
    setS(G.discardForCoins(s, selCard.id)); setSel(null)
  }
  function doWonder(wid: string) {
    if (!yourTurn || !selCard) return
    setS(G.buildWonder(s, selCard.id, wid)); setSel(null)
  }
  ...
  if (s.winner === 0) {
    ...
    banner = s.winBy === 'military' ? 'Military victory — capital seized!'
      ...
      : `You win — ${G.scoreVP(s, 0)} VP`
  } else if (s.winner === 1) {
    bk = 'lose'
    banner = s.winBy === 'military' ? 'Your capital fell — military defeat'
      : s.winBy === 'science' ? 'The AI mastered all six sciences'
      : `The AI wins — ${G.scoreVP(s, 1)} VP`
  } else if (yourTurn) {
    bk = 'you'
  ...
  return (
    <>
      <GameShell
        ...
        onNew={newGame}
        modeLeft={`Age ${ageLabel} · You ${G.scoreVP(s, 0)} VP · AI ${G.scoreVP(s, 1)} VP`}
        banner={banner}
        bannerClass={bk}
        modeRight={<>click a lit card · build / wonder / discard &nbsp; N · new</>}
      >
        <div className="swd-main">
          <MilitaryTrack pos={s.military} />

          <div className="swd-pyramid-wrap">
            <div className="swd-section-l">Age {ageLabel} — accessible cards are lit</div>
            <Pyramid s={s} sel={sel} yourTurn={yourTurn} onPick={pick} />
          </div>

          <ActionBar
            s={s} selCard={selCard} me={me} yourTurn={yourTurn}
            onBuild={doBuild} onDiscard={doDiscard} onWonder={doWonder} onClear={() => setSel(null)}
          />
        </div>

        <div className="side">
          <Tableau label="You" who="you" p={me} mil={s.military} forYou active={yourTurn} />
          <Tableau label="AI" who="ai" p={ai} mil={s.military} active={s.turn === 1 && s.winner == null} />
...
function Pyramid({ s, sel, yourTurn, onPick }: {
  s: SWDState; sel: string | null; yourTurn: boolean; onPick: (id: string) => void
}) {
      ...
      {rows.map((slotIdxs, r) => (
        <div className="swd-prow" key={r}>
          {slotIdxs.map((idx) => {
            ...
            const afford = G.canAfford(s, 0, card)
            return (
              <CardFace
                key={idx}
                card={card}
...
function ActionBar({ s, selCard, me, yourTurn, onBuild, onDiscard, onWonder, onClear }: {
  s: SWDState; selCard: Card | null; me: PlayerState; yourTurn: boolean
  onBuild: () => void; onDiscard: () => void; onWonder: (wid: string) => void; onClear: () => void
}) {
  if (!yourTurn) {
    return <div className="swd-actionbar idle"><span className="swd-control-hint">Waiting for the AI…</span></div>
  }
  if (!selCard) {
    return (
  ...
  const canBuild = G.canAfford(s, 0, selCard)
        ...
        <div className="swd-wonder-opts">
          ...
          <div className="swd-wonder-btns">
            {unbuiltWonders.map((w) => {
              ...
              const ok = G.canAffordWonder(s, 0, w)
              return (
...
function ResultModal({ s, onNew }: { s: SWDState; onNew: () => void }) {
  const won = s.winner === 0
  const reason = s.winBy === 'military'
    ? (won ? 'You marched on the enemy capital.' : 'The AI overran your capital.')
    : s.winBy === 'science'
      ? (won ? 'You assembled all six sciences.' : 'The AI assembled all six sciences.')
      : (won ? 'You amassed the most victory points.' : 'The AI amassed the most victory points.')
  return (
    <Modal
    ...
    >
      <div className="modal-body">
        <p style={{ textAlign: 'center', fontSize: 15 }}>
          <span style={{ color: won ? 'var(--you)' : 'var(--ink-2)', fontFamily: 'var(--display)', fontWeight: 700 }}>You {G.scoreVP(s, 0)} VP</span>
          {'   ·   '}
          <span style={{ color: won ? 'var(--ink-2)' : 'var(--foe)', fontFamily: 'var(--display)', fontWeight: 700 }}>AI {G.scoreVP(s, 1)} VP</span>
        </p>
