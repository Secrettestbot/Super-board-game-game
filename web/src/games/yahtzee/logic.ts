/* YAHTZEE — dice logic.
   Ported verbatim from design/examples/dice_yahtzee/yahtzee_logic.jsx; the IIFE +
   window.YahtLogic global became ESM exports and a YahtzeeState type. The pure,
   immutable transition functions (Object.assign(...) returning new state) are
   unchanged so they drop straight into React's setState.

   13 rounds. Each turn: roll five dice, re-roll any subset up to twice, then fill one
   open category. Upper bonus +35 at 63+. Extra five-of-a-kinds score +100. You vs an
   AI; higher grand total wins. */
...
/** A scorecard: category key -> points (null = open), plus the extra-Yahtzee bonus. */
export type Card = Record<string, number | null>
...
export function counts(d) { const c = [0, 0, 0, 0, 0, 0, 0]; for (const x of d) c[x]++; return c; }
function sum(d) { return d.reduce((a, b) => a + b, 0); }
export function score(cat, d) {
...
function newCard(): Card { const c: Card = {}; for (const x of CATS) c[x.k] = null; c.yBonus = 0; return c; }

export function makeGame(): YahtzeeState {
  return {
...
export function totals(card): Totals {
...
export function toggleHold(s, i) { if (!s.rolled || s.turn !== "you") return s; const held = s.held.slice(); held[i] = !held[i]; return Object.assign({}, s, { held }); }

export function pick(s, who, cat) {
