/* DOMINOES — UI (built for this codebase). Double-six block-with-draw, vs a greedy AI.
   Ivory tiles on green baize; click a playable tile, then pick an end if it fits both. */
...
import type { DomState, Tile, End, Placed } from './logic'
...
export function Dominoes() {
  const [s, setS] = useState<DomState>(() => DM.makeGame())
  const [showRules, setShowRules] = useState(false)
  const [sel, setSel] = useState<number | null>(null)   // tileId of the selected hand tile

  function newGame() { setS(DM.makeGame()); setShowRules(false); setSel(null) }

  useAITurn(!s.winner && s.turn === 'ai', () => setS(p => DM.aiStep(p)), { delayMs: 620 })
  ...
  const youCanMove = useMemo(() => yourTurn && s.hands.you.some(t => DM.canPlay(s.line, t)), [yourTurn, s.hands.you, s.line])
  const e = DM.ends(s.line)

  const selTile = sel != null ? s.hands.you.find(t => DM.tileId(t) === sel) : undefined
  ...
  function pickTile(t: Tile) {
    ...
    if (myEnds.length === 1) { setS(DM.play(s, 'you', t, myEnds[0])); setSel(null); return }
    setSel(prev => prev === DM.tileId(t) ? null : DM.tileId(t))   // toggle: needs end choice
  }
  function playEnd(end: End) {
    if (!selTile) return
    setS(DM.play(s, 'you', selTile, end)); setSel(null)
  }
  function stuckAction() {
    if (!yourTurn || youCanMove) return
    setS(s.boneyard.length ? DM.draw(s, 'you') : DM.pass(s, 'you'))
  }

  let banner: string, bk = ''
  if (s.winner === 'you') { bk = 'win'; banner = `You win — +${s.scores.you}` }
  else if (s.winner === 'ai') { bk = 'lose'; banner = `The rival wins — +${s.scores.ai}` }
        ...
        <div className="dm-wrap">
          <div className="dm-board">
            <div className="dm-rival">
              <span className="dm-rival-l">Rival holds</span>
              <div className="dm-backs">
                {s.hands.ai.map((_, i) => <div key={i} className="dm-back" />)}
              </div>
            </div>
        ...
        <div className="side">
          <div className="panel scoreboard">
            <div className={'sc you' + (s.turn === 'you' && !s.winner ? ' on' : '')}>
              <span className="sc-chip you"></span><span className="sc-name">You · Ivory</span><span className="sc-n">{s.scores.you}</span>
            </div>
            <div className={'sc ai' + (s.turn === 'ai' && !s.winner ? ' on' : '')}>
              <span className="sc-chip ai"></span><span className="sc-name">Rival · Ebony</span><span className="sc-n">{s.scores.ai}</span>
            </div>
          </div>
          <div className="panel stats">
            <div className="stat"><span>Your tiles</span><b>{s.hands.you.length}</b></div>
            <div className="stat"><span>Rival tiles</span><b>{s.hands.ai.length}</b></div>
...
function ResultModal({ s, onNew }: { s: DomState; onNew: () => void }) {
  const won = s.winner === 'you', draw = s.winner === 'draw'
  const pts = won ? s.scores.you : s.scores.ai
  return (
    <Modal
      eyebrow={draw ? 'Locked up' : won ? (s.reason === 'out' ? 'Dominoes!' : 'Lighter hand') : 'Out-bonED'}
      title={draw ? 'A Tie' : won ? 'You Win' : 'Rival Wins'}
      closeOnOverlay={false}
      actions={<button className="btn-modal" onClick={onNew}>Play again</button>}
    >
      <div className="finalsc">
        <span className="you">You {s.scores.you}</span>
        <span className="foe">Rival {s.scores.ai}</span>
      </div>
      {!draw && <p className="dm-final-note">{s.reason === 'out' ? 'Went out' : 'Game blocked'} — scored {pts} from the loser's hand.</p>}
    </Modal>
  )
}
  ...
  return (
    <Modal eyebrow="How to play" title="Dominoes" onClose={onClose}
      ...
      <div className="modal-body">
        ...
        <p>A round ends when someone lays their <b>last tile</b> ("dominoes!") or the game is <b>blocked</b> (both pass). The winner — the one who went out, or the lighter hand if blocked — scores the <b>sum of pips in the rival's hand</b>. This is a single round: that player wins.</p>
