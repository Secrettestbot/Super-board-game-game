/* BACKGAMMON — UI (built for this codebase). A walnut & green board on the framework shell,
   vs a 1-ply heuristic AI. You are White (moving down toward home points 1–6); the AI is
   Black. Roll, click a checker, then click a highlighted destination. The AI rolls and plays
   each checker on a timer (useAITurn re-armed on a sub-move tick, like The Crew). */
...
import type { BackgammonState, Move } from './logic'
...
export function Backgammon() {
  const [s, setS] = useState<BackgammonState>(() => BG.makeGame())
  const [showRules, setShowRules] = useState(false)
  const [sel, setSel] = useState<number | null>(null)   // selected source (point index or BAR)

  function newGame() { setS(BG.makeGame()); setShowRules(false); setSel(null) }

  // AI plays several checkers per turn — re-arm the timer on every sub-move (remaining/rolled/turn).
  const aiActive = !s.winner && s.turn === 'b'
  useAITurn(aiActive, () => setS(p => BG.aiStep(p)), {
    delayMs: 560,
    tick: `${s.turn}-${s.rolled}-${s.remaining.join('')}-${s.barB}-${s.offB}`,
  })
  useGameKeys({
    onNew: newGame,
  ...
  // legal moves for the human with current remaining dice (post "use-most-dice" filter)
  const usable = useMemo<Move[]>(() => (yourTurn && s.rolled) ? BG.usableMoves(s, 'w') : [], [yourTurn, s])
  ...
  function doRoll() { if (yourTurn && !s.rolled) { setSel(null); setS(BG.roll(s, 'w')) } }
  ...
  function clickDest(to: number) {
    ...
    if (!mv) return
    setS(BG.move(s, 'w', mv.from, mv.die))
    setSel(null)
  }
  ...
  const pipW = BG.pipCount(s, 'w'), pipB = BG.pipCount(s, 'b')

  let banner = '', bk = ''
  if (s.winner === 'w') { bk = 'win'; banner = 'You win — all 15 borne off' }
  else if (s.winner === 'b') { bk = 'lose'; banner = 'The rival wins — all 15 borne off' }
  else if (yourTurn && !s.rolled) { bk = 'you'; banner = 'Your turn — roll the dice' }
  else if (yourTurn && s.rolled) { bk = 'you'; banner = s.barW > 0 ? 'Re-enter from the bar' : (sel == null ? 'Pick a checker to move' : 'Choose a destination') }
  else { bk = 'foe'; banner = s.rolled ? 'The rival is moving…' : 'The rival is rolling…' }
  ...
  return (
    <>
      <GameShell
        ...
        onNew={newGame}
        modeLeft={`Off ${s.offW}/15 · ${s.offB}/15`}
        banner={banner}
        bannerClass={bk}
      ...
      >
        <div className="bg-wrap">
          ...
          />
          <DiceTray s={s} yourTurn={yourTurn} onRoll={doRoll} />
        </div>

        <div className="side">
          <div className="panel scoreboard">
            <div className={"sc b" + (s.turn === 'b' && !s.winner ? " on" : "")}>
              <span className="sc-checker b" /><span className="sc-name">Rival · Black</span>
            </div>
            <div className="sc-meta"><span>bar {s.barB}</span><span>off {s.offB}</span><span>pip {pipB}</span></div>
            <div className={"sc w" + (s.turn === 'w' && !s.winner ? " on" : "")}>
              <span className="sc-checker w" /><span className="sc-name">You · White</span>
            </div>
            <div className="sc-meta"><span>bar {s.barW}</span><span>off {s.offW}</span><span>pip {pipW}</span></div>
          </div>
...
function DiceTray({ s, yourTurn, onRoll }: { s: BackgammonState; yourTurn: boolean; onRoll: () => void }) {
  ...
  return (
    <div className="bg-dicetray">
      ...
      {!showRoll && s.dice.length === 0 && <div className="bg-dice-hint">rival's roll</div>}
    </div>
  )
}
...
function ResultModal({ s, onNew }: { s: BackgammonState; onNew: () => void }) {
  const won = s.winner === 'w'
  return (
    <Modal
      eyebrow={won ? 'Borne off' : 'Out-raced'}
      title={won ? 'You Win' : 'Rival Wins'}
      closeOnOverlay={false}
      actions={<button className="btn-modal" onClick={onNew}>Play again</button>}
    >
      <div className="finalsc"><span className="you">You {s.offW}/15</span><span className="foe">Rival {s.offB}/15</span></div>
    </Modal>
  )
}
  ...
  return (
    <Modal eyebrow="How to play" title="Backgammon" onClose={onClose}
      ...
      <div className="modal-body">
        <p>You are <b>White</b>, racing your 15 checkers <i>down</i> toward your home board (points 1–6) and bearing them off. The rival (Black) races the other way. First to bear off all fifteen <b>wins</b>.</p>
        ...
        <p>A checker on the bar must <b>re-enter</b> the rival's home board before you do anything else. You must use as many dice as you legally can.</p>
