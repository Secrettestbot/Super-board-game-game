...
//
// Ported from the design handoff's tweaks-panel.jsx. The cross-iframe host protocol
// (postMessage __activate_edit_mode / __edit_mode_set_keys / ...) is removed since this
// runs standalone — instead the panel manages its own open state and shows a small
// launcher button when closed. The control components are unchanged.
...
const __TWEAKS_STYLE = `
  .twk-launch{position:fixed;right:16px;bottom:16px;z-index:2147483646;
    ...
    padding:8px 14px;background:rgba(250,249,247,.9);color:#29261b;cursor:pointer;
    -webkit-backdrop-filter:blur(24px) saturate(160%);backdrop-filter:blur(24px) saturate(160%);
    box-shadow:0 6px 20px rgba(0,0,0,.16);
    font:600 11px/1 ui-sans-serif,system-ui,-apple-system,sans-serif;letter-spacing:.04em}
...
// ── TweaksPanel ──────────────────────────────────────────────────────────────
// Floating, draggable shell. Open by default; the close button hides it and a small
// launcher button brings it back.
  ...
  return (
    <TweakRow label={label}>
      <select className="twk-field" value={value} onChange={(e) => onChange(e.target.value)}>
        {normOpts(options).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
      </select>
    </TweakRow>
  )
}
